...
let path = {
	...
	src: {
		...
		html: [src_folder + "/**/*.html", "!" + src_folder + "parts/_*.html"],
...
function fontstyle() {
	let file_content = fs.readFileSync(src_folder + '/scss/libs/fonts.scss');
	if (file_content == '') {
		fs.writeFile(src_folder + '/scss/libs/fonts.scss', '', cb);
			...
			if (items) {
				...
				for (var i = 0; i < items.length; i++) {
					...
					if (c_fontname != fontname) {
						fs.appendFile(src_folder + '/scss/libs/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
					}
					c_fontname = fontname;
				}
			}
